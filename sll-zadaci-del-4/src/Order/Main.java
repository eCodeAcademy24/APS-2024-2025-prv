...
class Factory {
    ...
    public static void orders(SLL<Order> active, SLL<Order> shipping) {
        SLLNode<Order> iterator = active.getFirst().succ;
        SLLNode<Order> highestPriority = active.getFirst();

        while (iterator != null) {
            if (iterator.element.getPriority() < highestPriority.element.getPriority()) {
                highestPriority = iterator;
            }
            iterator = iterator.succ;
        }

        shipping.insertLast(active.delete(highestPriority));
    }
