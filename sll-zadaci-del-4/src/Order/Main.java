...
class Factory {
    ...
    public static void orders(SLL<Order> active, SLL<Order> shipping) {
        SLLNode<Order> iterator = active.getFirst();

        int min = 999999999;
        SLLNode<Order> minNode = null;

        while (iterator != null) {
            if (iterator.element.getPriority() < min) {
                min = iterator.element.getPriority();
                minNode = iterator;
            }
            iterator = iterator.succ;
        }

        shipping.insertLast(active.delete(minNode));
    }
